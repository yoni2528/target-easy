  ...
  return (
    <section ref={ref} className="py-16 md:py-28 bg-white overflow-hidden">
      <div className="max-w-5xl mx-auto px-4 md:px-6">
        <h2 className="text-2xl md:text-5xl font-black text-center mb-2"
        ...
        <p className="text-center mb-10 md:mb-14 text-base md:text-lg" style={{ color: "#86868b", opacity: visible ? 1 : 0, transition: "opacity 0.8s ease 0.2s" }}>
      ...
      <div className="relative" onTouchStart={onTouchStart} onTouchEnd={onTouchEnd}
        ...
        <div className="relative h-[420px] md:h-[480px]" style={{ maxWidth: 1000, margin: "0 auto" }}>
          {scenarios.map((s, i) => {
            const d = getOff(i);
            ...
            return (
              <div key={i} className="absolute inset-y-0 w-[280px] md:w-[320px]" style={{
                left: "50%",
                transform: `translateX(-50%) translateX(${d * (isMobile ? 220 : 280)}px) translateZ(${absD === 0 ? 30 : -60}px) rotateY(${d * -40}deg) scale(${isHov ? 1.06 : absD === 0 ? 1 : 0.85})`,
              ...
              >
                <div className="h-full flex flex-col items-center justify-center px-4 md:px-6"
                  ...
                  <div style={{ width: "100%", maxWidth: isHov ? 250 : 210, height: isHov ? 180 : 170, transition: "all 0.4s ease" }}>
        ...
        <button onClick={() => go(-1)}
          className="absolute top-1/2 -translate-y-1/2 right-2 md:right-12 w-11 h-11 flex items-center justify-center rounded-full z-20 cursor-pointer"
          style={{ background: "rgba(255,255,255,0.85)", backdropFilter: "blur(12px)", boxShadow: "0 1px 8px rgba(0,0,0,0.08)" }}>
          <span style={{ color: "#1d1d1f", fontSize: 18 }}>›</span>
        </button>
        <button onClick={() => go(1)}
          className="absolute top-1/2 -translate-y-1/2 left-2 md:left-12 w-11 h-11 flex items-center justify-center rounded-full z-20 cursor-pointer"
          style={{ background: "rgba(255,255,255,0.85)", backdropFilter: "blur(12px)", boxShadow: "0 1px 8px rgba(0,0,0,0.08)" }}>
          <span style={{ color: "#1d1d1f", fontSize: 18 }}>‹</span>
        </button>
      </div>
      ...
      <div className="flex justify-center gap-1 mt-6 md:mt-8">
        {scenarios.map((_, i) => (
          <button key={i} onClick={() => setActive(i)} className="transition-all duration-500 cursor-pointer py-2 px-1"
            style={{}}>
            <div style={{ width: i === active ? 28 : 8, height: 8, borderRadius: 4, background: i === active ? "#1d1d1f" : "#d2d2d7", transition: "all 0.5s" }} />
          </button>
        ))}
      </div>
